...
(function () {
    ...
    function enlargeProjectImage(e) {
        ...
        let projectImages = document.querySelector('#project-detail .demos .gallery');
        let imgCurrent;
        let imgIndexCurrent;
        ...
        projectImages.querySelectorAll('a').forEach(el => {
            ...
            if(imgCurrent.substr(imgCurrent.length - imgClicked.length, imgClicked.length) === imgClicked) {
        ...
        imgClickedEl = document.querySelector('.interactive-gallery a[active=true]');
        showImage = true;
    ...
    function updateActiveImage(activeImage) {
        // debugger;
        document.querySelectorAll('.interactive-gallery > a').forEach(el =>  el.removeAttribute('active') )
        document.querySelector('.interactive-gallery > a[index="' + activeImage.getAttribute('index') + '"]').setAttribute('active', 'true');
    ...
    function updateControlsState(imgCount, imgCurrent) {
        ...
        controlsEl && controlsEl.setAttribute('class', 'gallery-controls ' + controlsStateClass);;
    ...
    function controlGalleryClick(e) {
        e.preventDefault();
        let visibleEl = document.querySelector('.interactive-gallery > a[active="true"]').getAttribute('index');
        let scrollToEl;
        let imgCount = document.querySelectorAll('.interactive-gallery > a').length;

        if(e.target.className === "next") {
            scrollToEl = document.querySelector('.interactive-gallery > a[index="' + (parseInt(visibleEl) + 1) + '"]')
        } else {
            scrollToEl = document.querySelector('.interactive-gallery > a[index="' + (parseInt(visibleEl) - 1) + '"]')
        }
    ...
    async function loadProject(e) {
        ...
        projectContentCode.length && projectContentCode.forEach((block) => {
        ...
        if(document.querySelector('.codegithub')) {
            ...
            let gistFrameHTML = '<html><body style="margin: 0"><script src="https://gist.github.com/f1vlad/1cd967c9e84741a39f4311858890aa20.js"></script></body></html>';
