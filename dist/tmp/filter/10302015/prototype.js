...
//console.log($('div.fake-select2:not(.hidden)').length);
    ...
    $(document).on('click', '[ref=fakeSelect2liClickRow1]', function(){
        ...
        $(this).closest('[ref=filterBasicRowContainer]').attr('filter_tool_obj_store', filter_tool_obj_store).removeClass('one-column-row initial-column-row three-column-row-visible-three three-column-row-visible-tho');
    ...
    function group_summation_generate_data() {
        $( "[ref=groupedListWrap] > .summation-grouping" ).each(function(index) {
            ...
            //construct_fake_flexlist(null, $(this).find('.summation-grouping-list-view').find('.flex-list-view-content'), num);
        });
    }
    ...
    function filter_show(v){
        if(v) {
            ...
            if($('[ref=filterBasicRowContainerModel]').length === 0)
                $('.filter-rows-container.filter-basic').find('[ref=filterBasicRowContainer]').attr('group', '1').addClass('last-group').clone().insertAfter('[ref=filterBasicRowContainer]').addClass('model hidden initial-column-row').attr('ref', 'filterBasicRowContainerModel').removeClass('last-group');
            filter_row_id_gen();
            filter_row_serialize();
